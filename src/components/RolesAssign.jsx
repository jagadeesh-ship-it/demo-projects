...
//                 headers: { Authorization: `Bearer ${token}` },
//             });
//             setUsers(res.data.data || res.data); // supports Option 1 and fallback
//         } catch (err) {
//             console.error("Error fetching users:", err);
//         }
//     }, [token]);
...
const RoleAssign = () => {
    ...
    const fetchUsers = useCallback(async () => {
        try {
            ...
            setUsers(res.data.data || res.data); // Option 1 compatible
        } catch (err) {
            console.error("Error fetching users:", err);
        }
    }, [token]);
    ...
    const handleSave = async () => {
        if (!selectedUser?.id || !updatedRole || !updatedDept) return;
        try {
            await axios.put(
                `http://localhost:5000/api/persons/${selectedUser.id}`,
